...
class TestE2EUI(unittest.TestCase):
    ...
    def test_bienvenida_loads_and_subject_buttons_enter_content(self):
        at = self._run_app()
        self.assertEqual(at.radio[0].value, "🐉 Bienvenida")
        self.assertTrue(any("LAGRANGIANITOS" in md.value for md in at.markdown))

        # Clicking a subject button from Bienvenida enters the content view (Dashboard PAES)
    ...
    def test_dashboard_class_navigation_and_cronometro(self):
        at = self._run_app()
        # Enter content view via a subject button in Bienvenida
        at.button(key="bv_n").click().run(timeout=60)
    ...
    def test_pb02_and_pb03_quiz_feedback_is_dynamic(self):
        at = self._run_app()
        # Enter content view for Datos y Azar via subject button in Bienvenida
